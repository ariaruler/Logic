...
\begin{definition}
    ...
    for example $(A_0 \to A_1 ,\neg A_3)$ \\ we easily call WWF ``formula''  and also we
...
\textbf{``sentence''}. Now question is ``are very closed formulas proposition?''
...
\begin{definition}
    ...
    \[ 		R^{-1}=\{u \mid \exists x,y \Big( u=(x,y) \land x\in B \land y\in A \land (y,x)\in R \Big)\}   \]
    or i.e.
    \[ 		R^{-1}=\{(x,y) \mid x\in B \land y\in A \land (y,x)\in R\}   \]
    therefore.
...
\begin{definition}
    ...
    we call a function \textbf{f} injective from \textbf{A} to \textbf{B}
    when it's function from  \textbf{A} to \textbf{B} and it's injective i.e.
