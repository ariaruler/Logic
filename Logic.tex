...
\begin{Rule}
    \textbf{$\bot I$} (this rule is scheme it means it works everywhere for every obj.)
...
\begin{Rule}
    \textbf{$\bot E$} (this rule is scheme it means it works everywhere for every obj.)
    (see attention 1.1).
    for (rp): \\
    \begin{tikzpicture}
        % Vertical lines
...
%
\begin{definition}
    \textbf{Atomic formula} every single obj or two obj(conatants and variable) and a binary
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        as we explain in ``\textbf{Substitution for obj}''. if it's like $x = y$ or $x \in X$
        ($x,y$ can be both constants and variables). we define it like this.
            \[ (x=y)[a/z] := (x[a/z] = y[a/z]) \]
            \[ (x \in X)[a/z] := (x[a/z] \in X[a/z]) \]
...
\begin{definition}
    ...
    \\
    \[ \forall x P(x) : \bigwedge\limits_{i=0}^n P[a_{i}/x] \]
\end{definition}
...
\begin{definition}
    ...
    \\
    \[ \exists  x P(x) : \bigvee\limits_{i=0}^n P[a_{i}/x] \]
\end{definition}
...
\begin{definition}
    ...
    and
    \[   f(x) \rightarrow Func(f) \land \exists y (x,y) \in f \]
\end{definition}
...
\begin{theorem}
    ...
    \begin{tabular}{>{\centering\arraybackslash}m{7.5cm}>{\centering\arraybackslash}m{7.5cm}}
        \begin{fitchproof}[arrayenv=tabular]
            ...
            \have {2} {\bigwedge\limits_{i=0}^n \phi (a_{i}) }          \by{$\land I,2,3,...,n+2$}{}
            \have  {2} { \forall x \phi (x)}                  \by{$def. n+3$ }{}
        \end{fitchproof}
         &
        \begin{fitchproof}[arrayenv=tabular]
        ...
        \begin{fitchproof}[arrayenv=tabular]
            \have  {2} {\phi (t)}
            \have  {2} {\bigvee\limits_{i=0}^n \phi (a_{i})}   \by{$\lor I 1$}{}
            \have  {2} { \exists x \phi (x)}         \by{$def 2$}{}
        \end{fitchproof}
