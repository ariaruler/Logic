...
%
\begin{definition}
    \textbf{Atomic formula} every single obj or  or two obj(conatants and variable) and a binary
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item if $A ,B$ are two formulas then $A \bigcirc  B $ is a formula too.($\bigcirc $ can be $\to,\land ,\lor$).
    \end{enumerate}
...
\begin{definition}
    ...
    proven by tools introduced in chapter 1($I= , E= , I\in , E\in , I \bot , \bot E $).
    \\
...
\begin{definition}
    ...
    \begin{enumerate}
        \setlength{\itemsep}{0pt} % No extra space between items
        \item for atomic formulas we define it like this. if it's a single object we do it 
        as we explain in ``\textbf{Substitution for obj}''. if it's like $x = y$ or $x \in y$
        ($x,y$ can be both constants and variables). we define it like this.
            \[ (x=y)[a/z] := (x[a/z] = y[a/z]) \]
...
\begin{theorem}
    ...
    \begin{fitchproof}[arrayenv=tabular]
        ...
        \have  [m] {2} {\varphi}
        \close
        \have   {2} {\varphi \to \psi }                              \by{$\to, I1,n$ }{}
        \have   {2} {\psi \to \varphi }                              \by{$\to, In+1,m$}{}
        \have  {2} {(\varphi \to \psi)\land (\psi \to \varphi)}     \by{$\land I,m+1,m+2$}{}
...
\begin{lemma}
    ...
    \begin{fitchproof}[arrayenv=tabular]
        ...
        \have   {2} {y \notin \emptyset }                                    \by{$\forall E,3$ }{}
        \have  {2} {\bot}                                                  \by{$\bot I,4,5$ }{}
        \close
        \have  {1} {y \notin A }                                            \by{$\neg I$}{}
        \have  {1} {\forall y (y \notin A )}                                \by{$\forall I,7$}{}
    \end{fitchproof}
\end{lemma}
...
you can olso see by definition of ``inverse of a Relation'' and ``image'' this equation holds.
...
\begin{definition}
    \textbf{Term} we define term like this.
        ...
        \item  if $t_{0},...t_{n}$ are terms and ``$Func(f)$'' and ``$(t_{0},...t_{n}) \in Dom(f)$''. 
        then ``$f(t_{0},...t_{n})$'' is also a term.
    \end{enumerate}
...
\begin{definition}
    ...
    \begin{enumerate}
        \setlength{\itemsep}{0pt} % No extra space between items
        \item for atomic formulas we define it like this. if it's a single term we do it 
        as we explain in ``\textbf{Substitution for term}''. if it's like $t_0=t_1$ or $t_0 \in t_1$
        ($t_0 , t_1$ are terms). we define it like this.
