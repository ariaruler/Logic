...
\begin{axiom}
    \textbf{Weak comprehension} $(schema)$ this axioms says for every open proposition with just one variable
    like $\phi (x,x_0 , x_1, ... , x_n)$ there is a set such that every $x$ is a member of that set if and only if it holds
    $\phi (x,x_0 , x_1, ... , x_n)$.
    \\
...
assume ``$R=\{x|x \notin x\}$'' is a set.you can't prove or disprove ``$R \in
    R$''. (we don't need to prove that now.we will prove it in
``\textbf{'MetaLogic'}''.)
