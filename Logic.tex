...
\begin{example}
    \textbf{}for P,Q,R we can prove these.Proof Is Left As An Exercise To The Reader.\twemoji{1f600}
...
\begin{axiom}
    ...
    so we denote it as ``$\{ x | \phi (x,x_0 , x_1, ... , x_n)\}$''.
\end{axiom}
...
assume ``$R=\{x|x \notin x\}$'' is a set.you can't prove or disprove ``$R \in
    R$'' so ``$x not\in x$'' is not a proposition in ``set theorhy''. (we don't
need to prove that now.we will prove it in``\textbf{'MetaLogic'}''). (it's not
obvios in ``$x not\in x$'' witch set holds and witch not). (and likely ``$\{ X
...
\chapter{From natural numbers complex numbers }
